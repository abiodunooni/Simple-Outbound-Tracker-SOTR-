...
const CallType = styled.span<{ $type: string }>`
  ...
  background-color: ${props => {
    switch (props.$type) {
      case 'call': return '#dbeafe'
      case 'meeting': return '#dcfce7'
      case 'follow-up': return '#fef3c7'
...
const CallOutcome = styled.span<{ $outcome: string }>`
  ...
  color: ${props => {
    switch (props.$outcome) {
      case 'connected':
      case 'meeting-scheduled': return '#059669'
      case 'voicemail':
      case 'callback-requested': return '#d97706'
...
export const LeadDetailView: React.FC<LeadDetailViewProps> = observer(({ lead, onEdit, onClose }) => {
        ...
        <Section>
          ...
          <CallHistoryList>
            {callHistory.length === 0 ? (
              <EmptyState>
                No calls logged yet. Click "Log Call" to record your first interaction.
              </EmptyState>
            ) : (
              callHistory.map((call: CallLog) => (
                <CallHistoryItem key={call.id}>
                  ...
                  {call.nextAction && (
                    <CallNotes><strong>Next Action:</strong> {call.nextAction}</CallNotes>
                  )}
                  
                  {call.scheduledFollowUp && (
                    <CallNotes>
                      <strong>Follow-up Scheduled:</strong> {formatDateTime(new Date(call.scheduledFollowUp))}
                    </CallNotes>
                  )}
                </CallHistoryItem>
              ))
