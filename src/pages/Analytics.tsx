...
export const Analytics: React.FC = observer(() => {
  ...
  const outcomeStats = callLogs.reduce(
    (acc: Record<CallOutcome, number>, call) => {
  ...
  const totalDuration = callLogs.reduce(
    (sum: number, call) => sum + (call.duration ?? 0),
    0
  );
  const avgDuration =
  ...
  const conversionRate =
    totalCalls > 0 ? Math.round((meetingsScheduled / totalCalls) * 100) : 0;

  // Generate pie chart data for lead status
  const statusPieData = Object.entries(leadsByStatus)
    .map(([status, leads]) => ({
      label: status,
      value: (leads as Lead[]).length,
      color: LEAD_STATUS_COLORS[status as LeadStatus],
      percentage:
        totalLeads > 0 ? ((leads as Lead[]).length / totalLeads) * 100 : 0,
    }))
  ...
  const outcomeBarData = Object.entries(outcomeStats).map(
    ([outcome, count]) => ({
      label: formatOutcome(outcome),
      value: count as number,
      percentage:
        maxOutcomeCount > 0 ? ((count as number) / maxOutcomeCount) * 100 : 0,
  ...
  if (totalLeads === 0 && totalCalls === 0) {
    return (
      <Container>
        ...
        <EmptyState>
          No data available yet. Add some leads and log some calls to see your
          analytics.
        </EmptyState>
      </Container>
    );
